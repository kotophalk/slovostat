...
Иконки сайта `app/static/favicon.svg`, `favicon.ico`, `apple-touch-icon.png` (маунт `/static`) генерирует `brand/build.py` репозитория `delosvod` (знак семьи Делосвода: две строки текста и третья по «словам», акцент `#1A73E8`) — руками не править; тот же знак инлайном в `<h1>` шаблона.
